...
void FileUpload::postFile(QString url, QString path, QString token, QString disposition)
{
    ...
    //if(path.startsWith("content://")) path = path.remove(0,10);
    //content://com.android.providers.media.documents/document/image%3A835
    ...
    multiPart->setBoundary("------------------------yryeiyfjheytoetouogjeotvjeout");
    QHttpPart part;
    QFile *file = new QFile(path);
    part.setHeader(QNetworkRequest::ContentTypeHeader, QVariant("multipart/form-data;boundary=" + multiPart->boundary()));
    part.setHeader(QNetworkRequest::ContentDispositionHeader, QVariant("form-data; name=file; filename=assignment.pdf"));

    QString s = "Bearer ";
    s.append(token);
    qCritical() << "TOKEN: " << s.toUtf8() ;
    qCritical() << "TOKEN: " << s ;
    ...
    //multiPart->setProperty("file", file->readAll());
    //multiPart->set

    QNetworkRequest request;
    request.setUrl(QUrl(url));
    request.setRawHeader("Authorization", s.toUtf8());
    //request.setAttribute(QNetworkRequest::, file);
    prepairRequest(&request);
